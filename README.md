...
| 명령 | 동작 |
|---|---|
| `/status` | 현재 프리미엄·환율·포지션 상태 조회 (장외에도 동작, 현물은 마지막 체결가) |
| `/open SKHX` | 진입 완료로 표시 → 이후 청산 알림 활성화 |
| `/flat SKHX` | 청산 완료로 표시 → 이후 진입 알림 활성화 |

진입 알림이 와서 실제로 포지션을 잡았으면 반드시 `/open SKHX`(또는 `SMSN`)를 보내야
프리미엄이 0 근처로 내려왔을 때 청산 알림이 온다. 상태는 `state.json`에 저장되어 재시작해도 유지된다.
